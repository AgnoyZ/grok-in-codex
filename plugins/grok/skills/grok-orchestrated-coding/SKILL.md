...
5. **Do not write the same workspace while Grok runs.** No overlapping Codex edits, commits, or formatter passes on that tree. Prefer `background=true` and wait on `grok_status` / `grok_result`.
